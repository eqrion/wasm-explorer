  ...
  const toggleCollapse = useCallback(
    (index: number) => {
      ...
      if (newCollapsed[index]) {
        ...
        const otherVisibleIndices = newCollapsed
          .map((c, i) => (!c && i !== index ? i : -1))
          .filter((i) => i !== -1);
        ...
        const totalCurrentWidth = otherVisibleIndices.reduce(
          (sum, i) => sum + newWidths[i],
          0,
        );
        ...
        if (totalCurrentWidth > 0) {
          otherVisibleIndices.forEach((i) => {
            newWidths[i] =
              (newWidths[i] * (100 - widthToTake)) / totalCurrentWidth;
          });
        }
      }
