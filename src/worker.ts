...
import type { ModuleId, MessageId, MessageToWorker, MessageFromWorker } from './messages.js'
import { LoadedMessageId, MessageToWorkerKind, MessageFromWorkerKind } from './messages.js'

let Module = module.Module;

// workaround for https://github.com/Microsoft/TypeScript/issues/20595
declare function postMessage(message: MessageFromWorker, transfer?: Transferable[]): void;
...
addEventListener('message', ({ data }: { data: MessageToWorker }) => {
    ...
    try {
        ...
        case MessageToWorkerKind.Construct: {
            ...
            postMessage({ kind: MessageFromWorkerKind.Construct, id: data.id, moduleId });
            return;
        }
        ...
        case MessageToWorkerKind.PrintRich: {
            ...
            postMessage({ kind: MessageFromWorkerKind.PrintRich, id: data.id, result });
            return;
        }
        case MessageToWorkerKind.PrintPlain: {
            let module = modules[data.moduleId];
            let result = module.printPlain(data.range);
            postMessage({ kind: MessageFromWorkerKind.PrintPlain, id: data.id, result });
            return;
        }
        ...
        postMessage({kind: MessageFromWorkerKind.Exception, id: data.id, exception: err});
    }
});
